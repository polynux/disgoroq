...
func TestLoadServiceConfig(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expected: ServiceConfig{
				GroqAPIKey:        "",
				GroqModel:         "llama-3-70b-versatile",
		...
		},
		{
			...
			envVars: map[string]string{
				...
				"GROQ_VISION_MODEL":   "custom-groq-vision",
				"OLLAMA_ENABLED":      "true",
				"OLLAMA_MODEL":        "custom-ollama-model",
				"OLLAMA_VISION_MODEL": "custom-ollama-vision",
			},
			expected: ServiceConfig{
				GroqAPIKey:        "test-key-123",
				GroqModel:         "custom-groq-model",
				GroqVisionModel:   "custom-groq-vision",
				OllamaEnabled:     true,
				OllamaURL:         "http://localhost:11434",
				OllamaModel:       "custom-ollama-model",
				OllamaVisionModel: "custom-ollama-vision",
				RetryConfig:       DefaultRetryConfig(),
				MinResponseLength: 1,
				FallbackEnabled:   true,
			},
