...
class OfficialQwen3TTSBackend(TTSBackend):
    ...
    async def generate_speech(
        ...
        # Base models only support custom voices (voice cloning)
        # CustomVoice models support built-in speakers
        if self.get_model_type() == "base":
            raise RuntimeError(
                f"Base model requires a custom voice for speech generation. "
                f"Voice '{voice}' is not a loaded custom voice. "
                f"Available custom voices: {list(self._custom_voices.keys())}. "
                f"Use a CustomVoice model for built-in speakers."
            )

        try:
            # Generate speech - only for CustomVoice models
    ...
    async def load_custom_voices(self, custom_voices_dir: str) -> None:
        ...
        for entry in sorted(voices_path.iterdir()):
            ...
            if cache_path.exists():
                try:
                    # Use weights_only=False since we trust the cache files we create
                    # The VoiceClonePromptItem dataclass is not in torch's safe globals
